...
The RPM spec file provided here is for KringleProjectCoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://kringleprojectcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[KringleProjectCoin git repository](https://github.com/KringleProjectCoinProject/Kringleprojectcoin) but are not part of
...
Currently only `Source10` is used, for the example `kringleprojectcoin.conf` file.
...
in the kringleprojectcoin git repository. At present only an SVG version of the KringleProjectCoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the kringleprojectcoin
...
needed when 0.12.1 is released, a proper fix is already in the KringleProjectCoin git
master branch.
...
idea to build KringleProjectCoin Core against a static version of that library compiled
...
The `kringleprojectcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `kringleprojectcoin-server` package. At this point, most RPM based
...
The `kringleprojectcoin-server` package should have SELinux support. How to properly do
...
KringleProjectCoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/kringleprojectcoin/). He has been
packaging KringleProjectCoin for Fedora at least since 2012.
...
4. I renamed the `kringleprojectcoin` package that contains the Qt GUI to `kringleprojectcoin-core` as
that appears to be how the general population refers to it, in contrast to
`kringleprojectcoin-xt` or `kringleprojectcoin-classic`. I wanted to make sure the general population
