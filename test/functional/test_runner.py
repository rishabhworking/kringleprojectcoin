...
`test/functional/test_framework/test_framework.py:KringleProjectCoinTestFramework.main`.
...
#    'wallet_basic.py',
    ...
    'rpc_net.py',
    'interface_kringleprojectcoin_cli.py',
...
def main():
    ...
    tmpdir = "%s/kringleprojectcoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    enable_kringleprojectcoind = config["components"].getboolean("ENABLE_KPCD")

    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/KringleProjectCoinProject/Kringleprojectcoin/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/KringleProjectCoinProject/Kringleprojectcoin/pull/5677#issuecomment-136646964
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    if not (enable_wallet and enable_cli and enable_kringleprojectcoind):
        print("No functional tests to run. Wallet, utils, and kringleprojectcoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    # Warn if kringleprojectcoind is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "kringleprojectcoind"]) is not None:
            print("%sWARNING!%s There is already a kringleprojectcoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "KPCD" not in os.environ:
        os.environ["KPCD"] = build_dir + '/src/kringleprojectcoind' + exeext
        os.environ["KPCCLI"] = build_dir + '/src/kringleprojectcoin-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `kringleprojectcoin-cli help` (`rpc_interface.txt`).
