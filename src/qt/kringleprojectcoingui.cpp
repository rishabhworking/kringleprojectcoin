...
#include "kringleprojectcoingui.h"

#include "kringleprojectcoinunits.h"
...
const std::string KringleProjectCoinGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString KringleProjectCoinGUI::DEFAULT_WALLET = "~Default";

KringleProjectCoinGUI::KringleProjectCoinGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
KringleProjectCoinGUI::~KringleProjectCoinGUI()
{
...
void KringleProjectCoinGUI::loadFonts()
{
...
void KringleProjectCoinGUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a KringleProjectCoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and kringleprojectcoin: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to KPC addresses"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your KringleProjectCoin addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified KringleProjectCoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a kringleprojectcoin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible KringleProjectCoin command-line options").arg(tr(PACKAGE_NAME)));
...
void KringleProjectCoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void KringleProjectCoinGUI::createToolBars()
{
    if(walletFrame)
    {
        /** KPC START */
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/kringleprojectcointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* kringleprojectcoinLabelLayout = new QVBoxLayout(toolbarWidget);
        kringleprojectcoinLabelLayout->addWidget(label);
        kringleprojectcoinLabelLayout->addWidget(toolbar);
        kringleprojectcoinLabelLayout->setDirection(QBoxLayout::TopToBottom);
        kringleprojectcoinLabelLayout->addStretch(1);
...
void KringleProjectCoinGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool KringleProjectCoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool KringleProjectCoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void KringleProjectCoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void KringleProjectCoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void KringleProjectCoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void KringleProjectCoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void KringleProjectCoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void KringleProjectCoinGUI::optionsClicked()
{
...
void KringleProjectCoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void KringleProjectCoinGUI::showDebugWindow()
{
...
void KringleProjectCoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void KringleProjectCoinGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void KringleProjectCoinGUI::openClicked()
{
...
void KringleProjectCoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void KringleProjectCoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void KringleProjectCoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void KringleProjectCoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void KringleProjectCoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void KringleProjectCoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** KPC START */
void KringleProjectCoinGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void KringleProjectCoinGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void KringleProjectCoinGUI::gotoManageAssetsPage()
{
...
void KringleProjectCoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to KringleProjectCoin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void KringleProjectCoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void KringleProjectCoinGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void KringleProjectCoinGUI::updateHeadersSyncProgressLabel()
{
...
void KringleProjectCoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void KringleProjectCoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("KringleProjectCoin"); // default title
    ...
    // Append title to "KringleProjectCoin - "
    if (!msgType.isEmpty())
        strTitle += " - " + msgType;
...
void KringleProjectCoinGUI::changeEvent(QEvent *e)
{
...
void KringleProjectCoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void KringleProjectCoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void KringleProjectCoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "KPC")
        msg += tr("Amount: %1\n").arg(KringleProjectCoinUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(KringleProjectCoinUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void KringleProjectCoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void KringleProjectCoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool KringleProjectCoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool KringleProjectCoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void KringleProjectCoinGUI::setHDStatus(int hdEnabled)
{
...
void KringleProjectCoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void KringleProjectCoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void KringleProjectCoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void KringleProjectCoinGUI::detectShutdown()
{
...
void KringleProjectCoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void KringleProjectCoinGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void KringleProjectCoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(KringleProjectCoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void KringleProjectCoinGUI::subscribeToCoreSignals()
{
...
void KringleProjectCoinGUI::unsubscribeFromCoreSignals()
{
...
void KringleProjectCoinGUI::toggleNetworkActive()
{
    if (clientModel) {
    ...
    menu(0)
{
    ...
    QList<KringleProjectCoinUnits::Unit> units = KringleProjectCoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const KringleProjectCoinUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(KringleProjectCoinUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    for (KringleProjectCoinUnits::Unit u : KringleProjectCoinUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(KringleProjectCoinUnits::name(u)), this);
...
{
    setText(KringleProjectCoinUnits::name(newUnits));
}
...
void KringleProjectCoinGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=KPCBTC"));
