...
void RunTest(const TestVector &test)
{
    ...
    for (const TestDerivation &derive : test.vDerive)
    {
        ...
        // Test private key
        CKringleProjectCoinExtKey b58key;
        b58key.SetKey(key);
        BOOST_CHECK(b58key.ToString() == derive.prv);

        CKringleProjectCoinExtKey b58keyDecodeCheck(derive.prv);
        CExtKey checkKey = b58keyDecodeCheck.GetKey();
        assert(checkKey == key); //ensure a base58 decoded key also matches

        // Test public key
        CKringleProjectCoinExtPubKey b58pubkey;
        b58pubkey.SetKey(pubkey);
        BOOST_CHECK(b58pubkey.ToString() == derive.pub);

        CKringleProjectCoinExtPubKey b58PubkeyDecodeCheck(derive.pub);
