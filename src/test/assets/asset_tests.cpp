...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of KPCCOIN NOT allowed
        BOOST_CHECK(!IsAssetNameValid("KPC", type));
        BOOST_CHECK(!IsAssetNameValid("KPC", type));
        BOOST_CHECK(!IsAssetNameValid("KPCCOIN", type));

        //- Versions of KPCCOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("KPC.COIN", type));
        BOOST_CHECK(IsAssetNameValid("KPC_COIN", type));
        BOOST_CHECK(IsAssetNameValid("KPCSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERKPC", type));
        BOOST_CHECK(IsAssetNameValid("KPCSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDEKPC", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_KPCS", type));
        BOOST_CHECK(IsAssetNameValid("SEKPCOT", type));
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("KPC", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(Params().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
