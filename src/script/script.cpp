...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** KPC START */
    case OP_KPC_ASSET              : return "OP_KPC_ASSET";
    /** KPC END */
...
{
    if (this->size() > 30) {
        if ((*this)[25] == OP_KPC_ASSET) { // OP_KPC_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == KPC_R) { // Check to see if KPC starts at 27 ( this->size() < 105)
                if ((*this)[28] == KPC_V)
                    if ((*this)[29] == KPC_N)
                        index = 30;
            } else {
                if ((*this)[28] == KPC_R) // Check to see if KPC starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == KPC_V)
                        if ((*this)[30] == KPC_N)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == KPC_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == KPC_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == KPC_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == KPC_R) {
