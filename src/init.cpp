 ...
 * between kringleprojectcoind, and kringleprojectcoin-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("kringleprojectcoin-shutoff");
    ...
    FlushWallets();
#endif
    GenerateKringleProjectCoins(false, 0, Params());
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), KPC_CONF_FILENAME));
    if (mode == HMM_KPCD)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), KPC_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/KringleProjectCoinProject/Kringleprojectcoin>";
    const std::string URL_WEBSITE = "<https://kringleprojectcoin.org>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("kringleprojectcoin-loadblk");
...
/** Sanity checks
 *  Ensure that KringleProjectCoin is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("KringleProjectCoin version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single KringleProjectCoin process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", KPC_CONF_FILENAME)).string());
    ...
    GenerateKringleProjectCoins(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
