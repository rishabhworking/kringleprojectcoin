...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx)
{
    ...
    if (nHeight <= DEV_FUND_UNTIL) {
      ...
      coinbaseTx.vout[1].scriptPubKey = GetScriptForDestination(CKringleProjectCoinAddress(address).Get());
...
void static KringleProjectCoinMiner(const CChainParams& chainparams)
{
    LogPrintf("KringleProjectCoinMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kringleprojectcoin-miner");
    ...
    if (!EnsureWalletIsAvailable(pWallet, false)) {
        LogPrintf("KringleProjectCoinMiner -- Wallet not available\n");
    }
        ...
        while (true) {
            ...
            if (chainparams.MiningRequiresPeers()) {
                ...
                do {
                    ...
                    LogPrintf("KringleProjectCoinMiner -- waiting for connections and blocks downloaded\n");
                    MilliSleep(1000);
                } while (true);
            }
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("KringleProjectCoinMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("KringleProjectCoinMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
            ...
            while (true)
            {
                ...
                while (true)
                {
                    ...
                    if (UintToArith256(hash) <= hashTarget)
                    {
                        ...
                        LogPrintf("KringleProjectCoinMiner:\n  proof-of-work found\n  hash: %s\n  target: %s\n", hash.GetHex(), hashTarget.GetHex());
    ...
    {
        LogPrintf("KringleProjectCoinMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("KringleProjectCoinMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateKringleProjectCoins(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&KringleProjectCoinMiner, boost::cref(chainparams)));
    }
